...
// four to a hex digit, and "3" is the payload version. A lead can mark the
...
// prompt-formats section landed between L3 and TB) and v2 (eleven, before the
// adversarial section landed between L7 and the sources). A link a teammate
...
const FROZEN: Record<string, string[]> = { "1": V1_ORDER, "2": V2_ORDER };
...
export function readShareHash(sections: Section[]): SharePayload | null {
  const m = location.hash.match(/^#share=([123])\.([0-9a-f]+)$/i);
  if (!m) return null;
  const bits = [...m[2]]
